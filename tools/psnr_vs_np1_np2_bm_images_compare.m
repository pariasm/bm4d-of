...
% this scripts generates plots comparing the results obtained with filtering
% of the mean with those obtained without it.

stage1_folder_m1 = [root_folder 'table_1np_vs_2np_vs_1b_img-color_1thY_1muH_2muN_1px8_2px8/'];
...
for sidx = 1:length(sigmas), s = sigmas(sidx);
	...
	% determine the index corresponding to b1 = 1 in the reference method
	idx_beta_1 = find(b1s_m2 == 1);
	...
	m_m2_beta1 = max(max(eff_m2(:,idx_beta_1,:,sidx)));
...
%		end
		...
		for ii = length(nsims2_m2):-1:1,
			color = [color_step*(ii-1) 1-color_step*(ii-1) 1-color_step*(ii-1) ];
%			plot([0 10], [eff_m2(ii,idx_beta_1,jj,sidx) eff_m2(ii,idx_beta_1,jj,sidx)],'--', 'Color',color,'LineWidth', 2)
		end
		plot([0 10], [m_m2_beta1 m_m2_beta1],'k--', 'LineWidth', 2)
		axis([min(b1s_m2(1),b1s_m1(1))-.05 max(b1s_m2(end),b1s_m1(end))+.05 m-.4 m+.2])
		grid on
		hold off
		h_leg = legend(...
							'$n_2 = 60$',...
...
%	set(gca,'yticklabel', ylls(:,1:2));
	...
	m_m2_beta1 = max(max(ebb_m2(:,idx_beta_1,:,sidx)));
...
%		plot([0 10], [ebb_m2(ii,idx_beta_1,jj,sidx) ebb_m2(ii,idx_beta_1,jj,sidx)],'--', 'Color',color,'LineWidth', 2)
		plot([0 10], [m_m2_beta1 m_m2_beta1],'--', 'Color',color,'LineWidth', 2)
