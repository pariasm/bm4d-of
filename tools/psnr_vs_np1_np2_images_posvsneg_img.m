...
stage1_folder_neg = [root_folder 'table_1np_vs_2np_img-color_1cntrY_2cntrY_1px8_2px8_neg/'];


% stage 1 ---------------------------------------------------
nsims1 = [4 6 8 10 15 20 30 40 50 60 80 100];
nsims2_p = [4 6 8 10 15 20 30 40 50 60 80 100];
nsims2_n = [4 6 8 10 15 20 30 40 50 60 80 100];
seqs = {...
'alley',...
'book',...
'building1',...
...
eff_p = zeros(length(nsims1),length(nsims2_p),length(sigmas));
ebb_p = zeros(length(nsims1),length(nsims2_p),length(sigmas));

eff_n = zeros(length(nsims1),length(nsims2_n),length(sigmas));
ebb_n = zeros(length(nsims1),length(nsims2_n),length(sigmas));
...
for sidx = 1:length(sigmas), s = sigmas(sidx);
	...
	% load final neg
	ff_n = nan(length(nsims1),length(nsims2_n),length(seqs));
%	for i = 1:length(seqs),
...
%	end
	...
	% load basic neg
	bb_n = nan(length(nsims1),length(nsims2_n),length(seqs));
%	for i = 1:length(seqs),
...
%	end
	...
	% load final pos
	ff_p = nan(length(nsims1),length(nsims2_p),length(seqs));
	for i = 1:length(seqs),
	...
	% load basic pos
	bb_p = nan(length(nsims1),length(nsims2_p),length(seqs));
	for i = 1:length(seqs),
	...
	hold on
	color_step = 1/length(nsims1);
	for ii = length(nsims1):-1:1,
