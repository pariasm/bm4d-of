...
stage1_folder = [root_folder 'table_1r_vs_1np_mono-gtoracle/'];
stage2_folder = [root_folder 'table_2r_vs_2np_mono-gtoracle/'];
%stage2_folder = [root_folder 'table_2r_vs_2np_mono-gtoracle-2nditer/'];
...
% <--> % seqs = {'tennis_mono', 'mobile_mono', 'stefan_mono', 'gsalesman'};
% <--> % nseqs = length(seqs);
% <--> % 
% <--> % mff   = zeros(length(nsims),length(ranks),4);
% <--> % minff = zeros(length(nsims),length(ranks),4);
% <--> % 
...
% <--> % 		print(gcf, '-dpng', ['fpsnr_r1-np1-image_s' num2str(s) '_' seqs{i}]);
% <--> % 	end
% <--> % 
% <--> % 	mff(:,:,s/10) = mean(ff,3);
...
% <--> % 	print(gcf, '-dpng', ['fpsnr_r1-np1-image_s' num2str(s) '_average_mono']);
% <--> % 
% <--> % 	minff(:,:,s/10) = min(ff - ...
% <--> % 	   repmat(max(max(ff(:,1:end-4,:))),[length(nsims) length(ranks) 1]),[],3);
% <--> % 	eff = minff(:,:,s/10);% - max(mff(:,:,s/10));
% <--> % 
% <--> % 	m = max(max(eff(:,1:end-1)));
% <--> % 	imagesc(eff, m + [-2 0]),
% <--> % %	axis equal,
...
% <--> % 	xlabel('r')
% <--> % 	ylabel('n')
% <--> % 	print(gcf, '-dpng', ['fpsnr_r1-np1-image_s' num2str(s) '_min_mono']);
% <--> % 	print(gcf, '-depsc2', ['fpsnr_r1-np1-image_s' num2str(s) '_min_mono']);
% <--> % 
% <--> % end
% <--> % 
% <--> % mbb = zeros(length(nsims),length(ranks),4);
% <--> % 
% <--> % for s = 10:10:40,
% <--> % 
...
% <--> % %		bb(:,:,i) = tmp(:,round((ranks - 10)/5/4)*4 + 2);
% <--> % 		bb(:,ranks <= 55,i) = tmp(:,ranks <= 55);
% <--> % 	end
% <--> % 
% <--> % 	for i = 1:length(seqs),
...
% <--> % 		print(gcf, '-dpng', ['bpsnr_r1-np1-image_s' num2str(s) '_' seqs{i}]);
% <--> % 	end
% <--> % 
% <--> % 	mbb(:,:,s/10) = mean(bb,3);
% <--> % 	ebb = mbb(:,:,s/10);% - max(mff(:,:,s/10));
% <--> % 
...
% <--> % 	xlabel('r')
% <--> % 	ylabel('n')
% <--> % 	print(gcf, '-dpng', ['bpsnr_r1-np1-image_s' num2str(s) '_average_mono']);
% <--> % 	print(gcf, '-depsc2', ['bpsnr_r1-np1-image_s' num2str(s) '_average_mono']);
% <--> % 
% <--> % end
% <--> % 
% <--> % mtt = zeros(length(nsims),length(ranks),4);
% <--> % 
% <--> % for s = 10:10:40,
% <--> % 
% <--> % 	tt = zeros(length(nsims),length(ranks),nseqs);
% <--> % 
% <--> % 	for i = 1:length(seqs),
% <--> % 		tmp = load([stage1_folder 'table_time_' seqs{i} '_s' num2str(s)]);
% <--> % 		tmp(tmp == 0) = NaN;
% <--> % 
% <--> % 		tmp_l = tmp(:,2:4:end);
% <--> % 		tt(:,:,i) = interp2(rr_l, nn_l, tmp_l, rr_h, nn_h);
% <--> % 		tt(:,ranks <= 55,i) = tmp(:,ranks <= 55);
% <--> % 	end
% <--> % 
% <--> % 	mtt(:,:,s/10) = mean(tt,3);
% <--> % 	ett = mtt(:,:,s/10);% - max(mff(:,:,s/10));
% <--> % 
% <--> % 	imagesc(ett),
% <--> % %	axis equal,
% <--> % 	axis tight,
...
% <--> % 	xlabel('r')
% <--> % 	ylabel('n')
% <--> % 	print(gcf, '-dpng', ['time_r1-np1-image_s' num2str(s) '_average_mono']);
% <--> % 	print(gcf, '-depsc2', ['time_r1-np1-image_s' num2str(s) '_average_mono']);
% <--> % 
% <--> % end
...
%		print(gcf, '-dpng'  , ['fpsnr_r2-np2-image_s' num2str(s) '_' seqs{i}]);
	end
...
%	axis equal,
	...
	print(gcf, '-dpng'  , ['fpsnr_r2-np2-image_s' num2str(s) '_average_mono']);
